...
from .OpenPackProcessing import openpackLoader
from .USCDataProcessing import uscLoader
...
def loading_data(dataset, args):     
    ...
    padding, timespan, min_seq, min_samples, aug_method, aug_wise = args.padding, args.timespan, args.min_seq, args.min_samples, args.aug_method, args.aug_wise
    ...
    if dataset == 'doore':
        ...
        dataset_list =  uscLoader('data/USC-HAD/**/*.mat', timespan, min_seq)
    ...
    print("min, max length:", min(length_list), max(length_list))
    ...
    # for r1 in range(datalist.shape[0]):
    #     label_num = labellist[r1]
    #     for r2 in range(datalist.shape[2]):
    #         write_list.append([label_num]+ datalist[r1,:, r2].tolist())

    # print(np.array(write_list).shape)
    # import csv
    # with open(str(dataset)+'.csv', 'w', encoding='UTF8', newline='') as f:
    #     writer = csv.writer(f)
    ...
    return  types_label_list, datalist, labellist, length_list


# split data into train/validate/test 
def loading_data_by_split(dataset, args): 
    ...
    for i in range(len(dataset_list)):
        ...
        label_list.append(dataset_list[i].label)
        length_list.append(dataset_list[i].length)
        id_list.append(dataset_list[i].user_id)
    ...
    return  types_label_list, datalist.cuda(), labellist, id_list